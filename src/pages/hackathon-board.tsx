    ...
    return (
        <section className="mb-8 overflow-hidden rounded-lg border border-border bg-panel">
                ...
                {METRICS.map((m) => {
                    ...
                    return (
                        <Button
                            ...
                            className={`!min-h-0 flex-1 rounded-none border-t border-border px-4 py-3 text-sm font-bold first:border-t-0 sm:border-t-0 sm:border-l sm:first:border-l-0 ${
            ...
            <div className="px-3 pt-4 pb-4 sm:px-5">
                {entries.length === 0 ? (
                    <div className={empty}>No usage counted yet.</div>
                ) : (
                    <div className="flex flex-col gap-1">
                        {entries.map((e) => {
                            ...
                            return (
                                <div
                                    key={e.username}
                                    className="grid grid-cols-[2.5rem_8rem_1fr_5rem] items-center gap-2 rounded-md px-1 py-1.5 hover:bg-panel2"
                                >
                                    ...
                                    <a
                                        href={`/u/${e.username}`}
                                        className="truncate text-sm font-semibold text-text no-underline"
                                    >
                                        {e.username}
                                    </a>
                                    <svg
