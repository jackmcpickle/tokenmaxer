...
export const Pricing: FC<{ base: string }> = ({ base }) => (
    ...
    >
        <section className={hero}>
            <h1 className="reveal">Reference pricing</h1>
                ...
                <tbody>
                    {listPrices().map((p) => (
                        <tr key={p.id}>
                            ...
                            <td className={num}>{perMillion(p.input)}</td>
                            <td className={num}>{perMillion(p.output)}</td>
                            <td className={num}>{perMillion(p.cacheRead)}</td>
                            <td className={num}>{perMillion(p.cacheWrite)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
        ...
        <p className="mt-6 max-w-[65ch] text-[13px] leading-snug text-muted">
