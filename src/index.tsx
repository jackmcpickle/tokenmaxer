...
import { parseMetric, parseSourceParam, parseWindow } from "@/routes/leaderboard";
...
app.get("/", async (c) => {
  ...
  const [entries, models] = await Promise.all([
    getLeaderboard(c.env.DB, { window, metric, source, model, limit: 100 }, Date.now()),
...
app.get("/u/:username", async (c) => {
  ...
  if (!profile) {
    return c.html(
      <Layout title="Not found · TokenTally" base={base}>
        <h1>Builder not found</h1>
        <p class="sub">
          No one has claimed that username yet. <a href="/start">Claim it →</a>
        </p>
      </Layout>,
      404,
    );
  }
  return c.html(<ProfilePage base={base} profile={profile} />);
});
