...
mkdir -p ~/.tokentally
printf '%s' '{"apiBase":"https://tokenmaxer.quest","token":"tt_..."}' > ~/.tokentally/config.json
```
...
double-counts. Source: <https://github.com/jackmcpickle/tokentally>.
...
Cookies) into `~/.tokentally/config.json` as `"cursorCookie"`. It is sent only
